...
var snajax = require('./snajax.js');
var recordTypes = require('./sntypes.json');

/*
Notes:
Table to set breakpoint is named sys_js_breakpoint
implement setting service-now breakpoints in vscode!
*/
...
sncmder.prototype.init = function (vscode) {
    ...
    this.outputChannel = this.vscode.window.createOutputChannel('sncmder');
};
sncmder.prototype.getApplications = function () {
    var self = this;
    self.appList = [{
        'label': 'global',
        'sys_id': 'global'
    ...
    $sn('sys_app').getRecords('scope!=global').then(function (result) {
...
sncmder.prototype.getAjaxClient = function () {
    return new snajax.SnAjaxClient({
        host: this.appConfig.host,
        user: this.appConfig.username,
        pass: this.appConfig.password
    });
...
sncmder.prototype.evalScript = function () {
    var self = this;
    self.showQuickPick(self.appList).then(function (selected) {
        if (!selected) return;
        var scope = selected.sys_id;
        var script = self.vscode.window.activeTextEditor.document.getText();
        var ajaxClient = self.getAjaxClient();
        ajaxClient.login().then(function () {
            return ajaxClient.evalScript(script, scope);
        }).then(function (res) {
            self.outputChannel.show(true);
            self.outputChannel.appendLine(res);
        });
    });
};
sncmder.prototype.openDebugger = function() {
    var url = 'https://' + this.appConfig.host + '/$jsdebugger.do?sysparm_nostack=true';
    opn(url, {app: ["C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe", '--new-window']});
};
